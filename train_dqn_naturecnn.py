...
def preprocess_frame(frame_rgb):
    """Preprocess Atari frame to 84x84 grayscale as in Mnih et al. (2015)"""
    img = Image.fromarray(frame_rgb)
    img = img.crop((0, 30, 160, 210))  # crop score area
    img = img.convert("L")  # grayscale
    img = img.resize((84, 84), Image.BILINEAR)  # resize to 84x84
    return np.asarray(img, dtype=np.float32) / 255.0  # normalize to [0, 1]


def stack_frames(stacked_frames, cur_frame, is_new_episode):
    """Maintain a stack of the last 4 preprocessed frames."""
    if is_new_episode or stacked_frames is None:
        stacked_frames = deque(
            [np.zeros((84, 84), dtype=np.float32) for _ in range(4)], maxlen=4
        )
        for _ in range(4):
...
def create_q_network(num_actions):
    """Deep Q-Network architecture from Mnih et al. (2015)"""
    inp = Input(shape=(84, 84, 4), dtype=tf.float32)
    x = Conv2D(16, (8, 8), strides=4, activation="relu")(inp)
